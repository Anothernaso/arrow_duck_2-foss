...
@export var _states: AD_GameStates
@onready var _current_state: AD_GameStates.State

func _ready() -> void:
	set_state(_states.default_state)
	

func set_state(new_state: AD_GameStates.State) -> void:
...
func get_state() -> AD_GameStates.State:
